...
def parse_fmap(data, label):
    ...
    if idx < 0:
        sys.exit(f"❌ {label}: no FMAP found — is this a coreboot image?")
...
def main():
    ...
    if len(orig) != len(new):
        sys.exit(f"❌ sizes differ ({len(orig)} vs {len(new)} B) — both must share the same flash layout.")
    ...
    for r in regions:
        if r not in ao:
            sys.exit(f"❌ region '{r}' missing in the original — layout incompatible.")
        if r not in an:
            sys.exit(f"❌ region '{r}' missing in the new ROM — layout incompatible.")
        oo, osz = ao[r]
        no, nsz = an[r]
        if osz != nsz:
            sys.exit(f"❌ '{r}' sizes differ (orig 0x{osz:X} vs new 0x{nsz:X}) — not transferable.")
    ...
    if "COREBOOT" in an:
        ...
        if bytes(out[cbo:cbo + cbs]) != new[cbo:cbo + cbs]:
            sys.exit("❌ internal error: the COREBOOT region would have been modified — aborted.")
